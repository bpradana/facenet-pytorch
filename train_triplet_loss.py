...
parser = argparse.ArgumentParser(
    description="Training a FaceNet facial recognition model using Triplet Loss."
)
parser.add_argument(
    "--dataroot",
    "-d",
...
parser.add_argument(
    ...
    type=str,
    default="datasets/glint360k.csv",
    help="Path to the csv file containing the image paths of the training dataset",
)
parser.add_argument(
    "--epochs", default=150, type=int, help="Required training epochs (default: 150)"
)
parser.add_argument(
...
parser.add_argument(
    "--num_human_identities_per_batch",
    default=32,
    type=int,
    help="Number of set human identities per generated triplets batch. (Default: 32).",
)
parser.add_argument(
    "--batch_size", default=544, type=int, help="Batch size (default: 544)"
)
parser.add_argument(
    "--lfw_batch_size",
    default=200,
...
parser.add_argument(
    "--margin", default=0.2, type=float, help="margin for triplet loss (default: 0.2)"
)
parser.add_argument(
    "--image_size",
    default=140,
...
parser.add_argument(
    ...
    default="cuda",
    type=str,
    help="Device to train the model on: ('cuda','cpu','mps'), (default: 'cuda')",
)
...
def set_model_architecture(model_architecture, pretrained, embedding_dimension):
    if model_architecture == "resnet18":
        ...
        raise NameError("Model architecture not found!")
    print("Using {} model architecture.".format(model_architecture))

    return model


def set_model_gpu_mode(model, device="cuda"):
    ...
    if device == "cuda":
        ...
        return model, flag_train_multi_gpu, device_object
    elif device == "cpu":
        model = model.to(device)
        print("Using cpu for training.")
        return model, False, device_object
    elif device == "mps":
        model.to(device)
        print("Using metal performance shader for training.")
        return model, False, device_object
...
def validate_lfw(model, lfw_dataloader, model_architecture, epoch, device_object):
    model.eval()
    with torch.no_grad():
        ...
        for batch_index, (data_a, data_b, label) in progress_bar:
            data_a = data_a.to(device_object)
            data_b = data_b.to(device_object)
    ...
    try:
        ...
        plot_accuracy_lfw(
            ...
            epochs=epoch,
            figure_name="plots/accuracies_plots/lfw_accuracies_{}_epoch_{}_triplet.png".format(
                model_architecture, epoch
            ),
        )
...
def forward_pass(imgs, model, batch_size, device_object):
    imgs = imgs.to(device_object)
...
def main():
    ...
    model, flag_train_multi_gpu, device_object = set_model_gpu_mode(model, device)  # type: ignore
    ...
    if resume_path:
        if os.path.isfile(resume_path):
            ...
            print(
                "Checkpoint loaded: start epoch from checkpoint = {}".format(
                    start_epoch
                )
            )
        else:
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_idx, (batch_sample) in progress_bar:
            ...
            if use_semihard_negatives:
                ...
                first_condition = (
                    (neg_dists - pos_dists < margin).cpu().numpy().flatten()
                )
